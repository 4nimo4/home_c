...
{
	/*
	FILE *f = fopen("temperature_small.csv", "rb");
	fread(d->bytes, sizeof(d->bytes), 1, f);
	//fread(d->bytes + sizeof(d->_data.number), d->_data.number * sizeof(struct sensor), 1, f);
	fclose(f);
	*/
    ...
    char name[] = "temperature_small.csv";     // Имя файла с данными

    open = fopen(name, "rb");                    // Открываем файл для чтения
    if(open == NULL)
	{ 
	printf("File opening error\n");    // Сообщаем об ошибке и выводим остаток строки                           // Если файл не открылся — выходим
        return;
	}
    int Y, M, D, H, m, T, n_scan, i = 0;               // Переменные для хранения данных и количества считанных полей
