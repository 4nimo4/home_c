...
//-----------------------------------------------------------------
            Debugging
//-----------------------------------------------------------------
        Что такое debugging ?  Используем наш код для stack FIFO для домашки
//-----------------------------------------------------------------
От английского слова bug — поиск жуков 
Отладка — это процесс, в котором разработчик, используя различные
доступные ему инструменты, пытается изменить часть ранее
написанного кода, чтобы избавиться от ошибок, которые не были
обнаружены во время компиляции.

//-----------------------------------------------------------------------------
Существует несколько способов отладки:
Трассировка. Вывод сигнальных сообщений.
Логирование. Вместо использования только «stderr» в качестве механизма
вывода, можно отправлять сообщения на специальные системы
логирования.
Отладка с использованием отладчика. Преимущество такого рода отладки
состоит в том, что с вашей стороны не требуется никаких дополнительных
усилий для начала отладки кода.
*/
//-----------------------------------------------------------------------------
...
#include <stdint.h>
//-----------------------------------------------------------------------------
// 1. Трассировка. Вывод сигнальных сообщений.
//-----------------------------------------------------------------------------
#define DEBUG       //включение отладочной информации
//#define DEBUG     //отключение отладолчной информации    
#ifdef DEBUG
    #define D if(1) printf("This is debug message\n"); //при включении D; будет выводиться сообщение This is debug message
#else
    #define D if(0) printf("This is debug message\n"); //при включении D; будет выводиться сообщение This is debug message
#endif
...
int main()
{
    ...
    D; //вывод диагностической информации --------------------------------!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//-----------------------------------------------------------------------------
//Добавить элемент в очередь
//-----------------------------------------------------------------------------
//    for(int i = 1;i <= 5; i++) 
