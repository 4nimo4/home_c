...
// -----------------------------------------------------------------------------
// переменные для работы таймеров для управления светодиодами
// -----------------------------------------------------------------------------
#define SV_GREEN  3
#define SV_YELLOW 4
#define SV_RED    5

unsigned long myTimer1, myTimer2, myTimer3;
bool ledState1=0;
bool ledState2=0;
bool ledState3=0;
...
void setup() {
//настроим выводы для управления светодиодами
  pinMode(SV_GREEN, OUTPUT);
  pinMode(SV_YELLOW, OUTPUT);
  pinMode(SV_RED, OUTPUT);
...
void loop() {
  ...
  if(millis() - myTimer1 >= 1000){
    myTimer1 = millis();
    ledState1 =! ledState1;
    digitalWrite(SV_GREEN, ledState1);
  }

  if(millis() - myTimer2 >= 333){
    myTimer2 = millis();
    ledState2 =! ledState2;
    digitalWrite(SV_YELLOW, ledState2);
  }

  if(millis() - myTimer3 >= 2000){
    myTimer3 = millis();
    ledState3 =! ledState3;
    digitalWrite(SV_RED, ledState3);
  } 
  //---------------------------------------------------------------------------
}