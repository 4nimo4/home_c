/*
//------------------------------------------------------ 
//  Создание HTTP-сервера вариант 2 - кнопки на сервере
//------------------------------------------------------
//     Создайте HTML-страницу с кнопками вида:
//------------------------------------------------------
Для этого потребуются теги:
● <html> - контейнер, который заключает в себе все
содержимое веб-страницы
● <head> (<h>) - заголовок
● <a > - нужен для создания гиперссылок, которые связывают
страницу с другой страницей или с определенным местом на
той же веб-странице.
● <button> - создает кликабельную кнопку, которая может
запускать действия или события на веб-странице
Добавьте диагностику подключения – при желании отключения от
сокета пользователь вводит в консоль символ “#ˮ.
...
#define PORT 8000       // TCP-порт, на котором будет работать HTTP-сервер
...
int main(void)
{
    ...
    while (1) {
        ...
        // Подготовим человекочитаемое отображение текущего состояния
